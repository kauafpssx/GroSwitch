...
import { fetchKeys, fetchStats, addKey, updateKey, deleteKey, resetTokens } from './api';
import { fetchModels, updateDefaultModel } from '@/features/models/api';
import { KeyCard } from './components/KeyCard';
import { AddKeyForm } from './components/AddKeyForm';
import { DefaultModelSelect } from '@/features/models/components/DefaultModelSelect';
...
import { Shield, AlertTriangle, Activity, Gauge, Cpu, TrendingDown, CalendarDays, Coins, RotateCcw } from 'lucide-react';
import { UnauthorizedError } from '@/shared/lib/http';
import { handleUnauthorized } from '@/shared/lib/auth';
import type { ApiKeyPublic, KeyStats, ModelRateLimitPublic } from '@groswitch/common';
...
export function Dashboard({ onLogout }: DashboardProps) {
  ...
  const handleUpdate = async (id: string, changes: { name: string; key: string }) => {
    await updateKey(id, changes);
    await loadKeys();
  };
  ...
  const handleResetTokensConfirm = async () => {
    await resetTokens();
  ...
  const anyCoolingDown = keys.some((k) => k.cooldownRemainingMs !== null && k.cooldownRemainingMs > 0 && k.limitedUntil);

  // Combined capacity across every registered key for the current default
  // model (each key carries its own independent Groq quota).
  const defaultModelTpm = models.find((m) => m.model === stats?.defaultModel)?.tpm ?? 0;
  const totalDailyCapacity = keys.length * (stats?.dailyLimit ?? 0);
  const totalRpmCapacity = keys.length * (stats?.minuteLimit ?? 0);
  const totalTpmCapacity = keys.length * defaultModelTpm;
  const requestsRemaining = Math.max(0, totalDailyCapacity - totalUsed);
  ...
  return (
    <div className="p-6 overflow-y-auto h-full">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div>
          ...
          {loading && keys.length === 0 ? (
            ...
            <div className="text-center py-12 border border-dashed border-border rounded-lg">
              ...
              <p className="text-muted-foreground/60 text-xs mt-1">Add a Groq API key to get started</p>
            </div>
          ) : (
        ...
        {/* Combined Capacity */}
        <div>
          <h2 className="text-sm font-medium mb-3">Combined Capacity</h2>
          <div className="grid grid-cols-4 gap-4">
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm flex items-center gap-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold">{totalUsed.toLocaleString()}/{totalDailyCapacity.toLocaleString()}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold">{totalRpmCapacity.toLocaleString()}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold">{totalTpmCapacity.toLocaleString()}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold">{requestsRemaining.toLocaleString()}</div>
              </CardContent>
            </Card>
          </div>
        </div>
      ...
      {/* Reset Tokens Confirmation Dialog */}
      <Dialog open={confirmResetTokens} onOpenChange={setConfirmResetTokens}>
        <DialogContent className="sm:max-w-[360px]">
          <DialogHeader>
            <DialogTitle>Reset Tokens</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground">
            Are you sure? This resets the token counter to zero for every key. This can't be undone.
          </p>
          <DialogFooter>
            <Button variant="outline" onClick={() => setConfirmResetTokens(false)}>Cancel</Button>
            <Button variant="destructive" onClick={handleResetTokensConfirm}>Reset</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
