...
import type { ModelRateLimit } from '@gemrouter/common';

function loadCsvRateLimits(): Record<string, { rpm: number; rpd: number; tpm: number }> {
  ...
  for (const line of lines) {
    ...
    if (model) {
      limits[model.trim()] = {
        rpm: parseInt(rpm, 10) || 30,
        rpd: parseInt(rpd, 10) || 1000,
        tpm: parseInt(tpm, 10) || 8000,
      };
    }
  }
  return limits;
...
export const modelRateLimitRepository = {
  async findOrCreate(model: string): Promise<ModelRateLimit> {
    ...
    const defaults = RATE_LIMITS[model] || { rpm: 30, rpd: 1000, tpm: 8000 };
  ...
  getDefaults(): Record<string, { rpm: number; rpd: number; tpm: number }> {
    return { ...RATE_LIMITS };
  },
